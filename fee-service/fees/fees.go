...
func (fo FeeOperations) RespondFees(w http.ResponseWriter, request *http.Request) {
	...
	if *feesAndAmountresponse.DetailsAssetSettlement.AssetCode == *feesAndAmountresponse.AssetCodePayout {
		...
		if *feesAndAmountresponse.AmountSettlement != *feesAndAmountresponse.AmountPayout {
			msg := "Warning: Settlement asset is same as payout asset, price rate should be the same, only fee should be charged, invalidating RFI's callback response"
...
func (fo FeeOperations) GetMyFee(feesCallbackURL string, feesAndAmountRequest model.FeesRequest) (model.TransactionFees, error) {

	LOGGER.Infof("In fee-service:GetMyFee: feesCallbackURL: ", feesCallbackURL)
	bodyBytes, err := json.Marshal(&feesAndAmountRequest)
	if err != nil {
		LOGGER.Errorf("Error while marshalling to JSON:  %v", err)
		return model.TransactionFees{}, err
	}
	/*
		var rq model.FeesAndAmountRequest
		err = json.Unmarshal(bodyBytes, &rq)

		if err != nil {
			LOGGER.Warningf("Unable to unmarshall to FAR: %v", err.Error())
		}
	*/

	req, _ := http.NewRequest("POST", feesCallbackURL, bytes.NewBuffer(bodyBytes))
	req.Header.Set("Content-Type", "application/json")
	res, err := fo.HTTPClient.Do(req)
	if err != nil {
		LOGGER.Errorf("Error calling callback fees  %v", err.Error())
		return model.TransactionFees{}, err
	}
	if res.StatusCode != http.StatusOK {
		LOGGER.Infof("Response from Fee Callback is: %v", res.StatusCode)
		return model.TransactionFees{}, err
	}
	...
	var fees model.TransactionFees
	readbuff, _ := ioutil.ReadAll(res.Body)
	err = json.Unmarshal(readbuff, &fees)
	if err != nil {
		LOGGER.Errorf("Error unmarshalling callback fees response  %v", err.Error())
		return model.TransactionFees{}, err
	}
	return fees, nil
}
