...
export class TokenComponent implements OnInit, OnDestroy {
  ...
  getTokens(): Observable<void> {
    ...
    return new Observable((observer: Observer<void>) => {
      this.portalApiService.getJWTInfoOfInstitution(this.sessionService.institution.info.institutionId).subscribe((jwtPublicInfos) => {

          const jwt_infos: { [iid: string]: IJWTPublic } = jwtPublicInfos ? this.portalApiServiceHelper.transformJwtInfoPortalApiEntityToIJWTPublic(jwtPublicInfos) : null;
  ...
  */
  ...
  /**
   * Controls whether or not logged in user can view this token's information
   *
   * @param {string} isCreator
   * @returns
   * @memberof TokenComponent
   */
  userCanView(isCreator: string) {

    // allowed users to view token:
    // - creator of token
    // - participant admin (approver/rejector)
