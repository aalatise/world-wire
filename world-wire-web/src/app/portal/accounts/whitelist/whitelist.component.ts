...
export class WhitelistComponent implements OnInit, OnDestroy {
  ...
  async getWhitelistRequests(): Promise<void> {
    ...
    return new Promise((resolve) => {
      this.portalApiService.getWhiteListRequests(this.sessionService.currentNode.participantId).subscribe(async (whitelistRequests) => {

        let allRequests: { [key: string]: WhitelistRequest } = {};
        if (whitelistRequests.length > 0) {
          whitelistRequests.forEach((request) => {
            allRequests = {
              ...allRequests,
              ...{[request.whitelistedId]: request},
            };
          });
        }
  ...
  getApprovalInfo(request: WhitelistRequest, approvalId: string, action: 'add' | 'delete'): Promise<WhitelistRequest> {
    return new Promise((resolve) => {
      this.portalApiService.getParticipantApproval(approvalId).subscribe(async (approval) => {
        if (approval) {
  ...
  async rejectWhitelistRequest(request: WhitelistRequest) {
    ...
    if (request.requestedBy !== this.authService.userProfile.profile.email) {
      ...
      this.portalApiService.updateWhiteListRequest(request.whitelistedId, {
        approvalIds: request.approvalIds,
        whitelisterId: request.whitelisterId,
        whitelistedId: request.whitelistedId,
        status: 'rejected',
  ...
  async approveDeleteWhitelistRequest(request: WhitelistRequest) {
    ...
    if (request.deleteRequestedBy !== this.authService.userProfile.profile.email) {
      try {
        ...
        if (response.ok) {
          await this.portalApiService.deleteWhiteListRequestP(request.key);
        }
        this.loadingRequest = false;
      } catch (err) {
        this.loadingRequest = false;
  ...
  resetApprovals(request: WhitelistRequest) {
    ...
    this.portalApiService.updateWhiteListRequest(request.key, {
      whitelisterId: request.whitelisterId,
      whitelistedId: request.whitelistedId,
