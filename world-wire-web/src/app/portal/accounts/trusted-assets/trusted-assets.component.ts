...
export class TrustedAssetsComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    this.model.header = [
      ...
      // TODO: add asset limit after https://github.ibm.com/gftn/world-wire-services/issues/893 is completed
      // new TableHeaderItem({
      //   data: 'Limit'
      // }),
      new TableHeaderItem({
      ...
      .subscribe((requests: TrustRequest[]) => {
        ...
        if (requests) {
          requests.forEach((request: TrustRequest) => {
            ...
            if (request.approval_ids && request.approval_ids.length > 1) {
              this.portalApiService.getParticipantApproval(request.approval_ids[1]).subscribe((approval) => {
