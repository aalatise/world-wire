...
export class TransactionService {
  ...
  public readonly transactionKeys: { [key: string]: TransactionKeyBook } = {
    'transfer': {
      ...
      settlementAssetCode: this.transactionDetails + '.asset_settlement.asset_code',
  ...
  public loadTransactions(
    ...
    const source = new Observable((observer: Observer<(ITransfer | IExchange)[]>) => {
      ...
      this.portalApiService.getTransactions(currentNode.participantId, transactionType).subscribe((transactionData) => {
        let transactions: (ITransfer | IExchange)[];
        switch (transactionType) {
          case 'exchange':
            transactions = this.processExchangeTransactions(currentNode.participantId, transactionData);
            break;
          default:
            transactions = this.processTransferTransactions(currentNode.participantId, transactionData);
            break;
        }
    ...
    transactionData.forEach((transaction) => {
      ...
      if (transaction) {
        ...
        if (institution === ofi || institution === rfi) {
          ...
          // TODO: change this to .account_name_receive when https://github.ibm.com/gftn/world-wire-services/issues/1071 is done
          transaction.account_name = transaction.reversed ?
          ...
          transaction.current_status = statusArray[statusArray.length - 1].transactionstatus;
