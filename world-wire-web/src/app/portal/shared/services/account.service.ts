...
export class AccountService {
  ...
  async getParticipant(): Promise<Participant> {
    ...
    try {
      ...
      let accountsRequest = `${ENVIRONMENT.envApiRoot}/${this.sessionService.currentNode.participantId}/api/v1/client/participants/${this.sessionService.currentNode.participantId}`;

      // use different URL to get participant details for super admins
      if (this.authService.userIsSuperUser()) {
        accountsRequest = `${ENVIRONMENT.envGlobalRoot}/pr/v1/admin/pr/domain/${this.sessionService.currentNode.participantId}`;
      }

      if (this.sessionService.currentNode.role === 'IS') {
        accountsRequest = `${ENVIRONMENT.envGlobalRoot}/anchor/v1/anchor/participants/${this.sessionService.currentNode.participantId}`;
      }
      ...
      // reset API roots
      this.apiRoot = `${ENVIRONMENT.envApiRoot}/${this.sessionService.currentNode.participantId}/api`;

      this.globalRoot = `${ENVIRONMENT.envGlobalRoot}`;
 ...
 */
  ...
  getAccountRequests(): Observable<AccountRequest[]> {
    return new Observable((observer: Observer<AccountRequest[]>) => {
      this.portalApiService.getAllAccountRequests(this.sessionService.currentNode.participantId).subscribe((accountRequests) => {
        const allRequests: AccountRequest[] = accountRequests;
        // refresh the view
        this.ngZone.run(() => {
  ...
  getTrustRequests(requestField: string, statusFilters?: TrustRequestStatus[]): Observable<TrustRequest[]> {
    return new Observable((observer: Observer<TrustRequest[]>) => {
      this.portalApiService.getAllTrustRequests(this.sessionService.currentNode.participantId, requestField).subscribe((trustRequests) => {
          let filteredRequests: TrustRequest[] = [];

          for (const [key, request] of Object.entries(trustRequests)) {

            // temporarily store key for future querying
            request.key = trustRequests[key]._id;
  ...
  async getAllParticipants(): Promise<Participant[]> {
    ...
    let accountsRequest = `${ENVIRONMENT.envApiRoot}/${this.sessionService.currentNode.participantId}/api/v1/client/participants`;

    // different endpoint for IS/Anchor
    if (this.sessionService.currentNode.role === 'IS') {
      accountsRequest = `${ENVIRONMENT.envGlobalRoot}/anchor/v1/anchor/participants`;
    }
