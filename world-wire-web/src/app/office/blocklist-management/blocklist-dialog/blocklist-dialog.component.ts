...
export class BlocklistDialogComponent implements OnInit {
  ...
  public submitRequest() {
    ...
    this.blocklistService.addToBlocklist(this.request, 'request').then(response => {
      const approvalId: string = response.msg ? response.msg : '';

      // log reference to approval in blocklist request
      if (approvalId) {
        this.request.approvalId = approvalId;

        const reqSub = this.portalApiService.createBlocklistRequest(this.request).subscribe(() => {
  ...
  public removalRequest() {
    ...
    this.blocklistService.removeFromBlocklist(this.request, 'request').then(response => {
      const approvalId: string = response.msg ? response.msg : '';
