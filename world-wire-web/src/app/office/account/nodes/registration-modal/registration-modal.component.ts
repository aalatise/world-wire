...
import { Participant, IInstitution } from '../../../../shared/models/participant.interface';
...
import { PortalApiService, PortalApiServiceModelTransformer } from '../../../../shared/services/portal-api.service';
...
export class RegistrationModalComponent extends BaseModal implements OnInit {
  ...
  async ngOnInit() {
    ...
    const accountsRequest = `${ENVIRONMENT.envGlobalRoot}/pr/v1/admin/pr/domain/${this.node.participantId}`;
  ...
  async rejectRegistration() {
    ...
    if (this.latestApproval) {
      ...
      let  institution = await this.portalApiService.getInstitutionP(this.node.participantId);
      institution = this.portalApiServiceHelper.updateOrAddInstitutionNode(institution, this.node.participantId, {
        issuingAccount: null,
  ...
  async submitForm(f: NgForm, permission: ApprovalPermission) {
    ...
    const request = `${ENVIRONMENT.envGlobalRoot}/anchor/v1/admin/anchor/${this.officialParticipant.id}/register`;
    ...
    apiRequest.then(async (response: any) => {
      ...
      if (permission === 'approve') {
        ...
        this.portalApiService.updateSuperApproval(this.node.accountApprovalId, {
          uid_approve: null,
          status: 'request'
        }).subscribe();
