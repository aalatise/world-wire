...
import { PortalApiService, PortalApiServiceModelTransformer } from '../../services/portal-api.service';
...
export class NodeConfigFormComponent implements OnInit, OnChanges {
  ...
  public async sendFormData(f: NgForm): Promise<void> {
    ...
    if (this.validForm(f)) {
      ...
      const automationUrl = `${ENVIRONMENT.envDeploymentRoot}`;

      const latestApprovalId = this.latestApproval ? this.latestApproval.key : null;

      let h: HttpHeaders = await this.authService.createSuperUserHeaders(this.configData.institutionId, this.configData.participantId);
      ...
      setTimeout(() => {
        apiRequest.then(async (response: any) => {
          ...
          if (permission === 'request' && response.msg) {
            let institution = await this.portalApiService.getInstitutionP(this.institution.info.institutionId);
            // create new record, if does not exist
            if (!this.configData.approvalIds) {
              institution = this.portalApiServiceHelper.updateOrAddInstitutionNode(institution, node.participantId, node);
              await this.portalApiService.updateInstitutionP(institution.info.institutionId, institution);
            }
            ...
            institution = this.portalApiServiceHelper.updateOrAddInstitutionNode(institution, node.participantId, {
          ...
          if (permission === 'approve') {
            ...
            // NOTE: get institution endpoint
            let institution = await this.portalApiService.getInstitutionP(this.institution.info.institutionId);
            const institutionNode = this.portalApiServiceHelper.getInstitutionNode(institution, node.participantId);

            const currentNodeConfig: INodeAutomation = institutionNode ? institutionNode : null;
            ...
            if (currentNodeConfig) {
              ...
              if (requestStatuses.length > 0 && !requestStatuses[0].includes('failed') && requestStatuses[0] !== 'ready') {
                requestStatuses[0] = 'configuration_failed';

                institution = this.portalApiServiceHelper.updateOrAddInstitutionNode(institution, node.participantId, { status: requestStatuses});
                this.portalApiService.updateInstitution(institution.info.institutionId, institution).subscribe();
              }
            }
          }

          if (error.error.service && error.error.service === 'automation-service') {
            let institution = await this.portalApiService.getInstitutionP(this.institution.info.institutionId);
            institution = this.portalApiServiceHelper.updateOrAddInstitutionNode(institution, node.participantId, { update: null});
            this.portalApiService.updateInstitution(institution.info.institutionId, institution).subscribe();

            // Prevent user from clicking the button for special automation service error.
            this.success = true;
          }
  ...
  public async rejectFormRequest(approvalId: string) {
    if (isDevMode) {
    ...
    let institution = await this.portalApiService.getInstitutionP(this.institution.info.institutionId);
    institution = this.portalApiServiceHelper.updateOrAddInstitutionNode(institution, this.configData.participantId, {
      status: ['deleted'],
      update: null,
  ...
  firstStatusIsReady(statuses: string[]): boolean {
    return statuses.length > 0 && (statuses[0] === 'ready');
  }
}
