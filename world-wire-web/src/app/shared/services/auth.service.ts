...
export class AuthService implements OnDestroy {
    ...
    async getFirebaseIdToken(institutionId?: string, participantId?: string): Promise<HttpHeaders> {
        ...
        const fid = this.session.accessToken;
    ...
    /**
     * Construct headers necessary for a super user
     * @param institutionId
     * @param participantId
     */
    public createSuperUserHeaders(institutionId?: string, participantId?: string): HttpHeaders {

        const fid = this.session.accessToken;

        let headers = new HttpHeaders();
        headers = headers.set('X-fid', fid);

        // institutionId used by participant permissions lookup in back-end
        if (institutionId) {
            headers = headers.set('x-iid', institutionId);
        }

        // if (participantId) {
            // headers = headers.set('x-pid', participantId);
        // }
    ...
    getUserProfile(authService?: AuthService): Promise<IUserProfile> {
      ...
      return new Promise((resolve, reject) => {
        if (authService && authService.userProfile) {
          resolve(authService.userProfile);
        } else {
          this.session.accessToken = localStorage.getItem('id_token');
          const token              = localStorage.getItem('id_token');

          const decryptedData: { userId: string } & any = new JwtHelperService().decodeToken(token);

          const userSub = this.portalApiService.getUserProfile(decryptedData.uid).subscribe(userProfile => {
            this.userProfile = this.portalApiServiceHelper.transformUserPortalApiEntityToIUserProfile([userProfile])[0];
    ...
    hasSpecificSuperPermissions(
        ...
        for (let i = 0; i < allowableSuperPermissions.length; i++) {
            ...
            if (userProfile.super_permission.roles[allowableSuperPermissions[i]]) {
                // permissions allowed!
                return true;
            }
        }
