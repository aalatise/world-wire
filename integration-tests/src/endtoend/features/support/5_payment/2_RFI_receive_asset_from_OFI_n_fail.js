...
const createReceivePayload = require('../../../utility/createReceivePayload')
...
Then('id: {string} participant_bic: {string} finished federation and compliance check response to send_participant: {string} send_participant_bic: {string} with federation_status: {string} compliance_status: {string} compliance_status: {string} receive asset_code: {string} account_name: {string} settlement_method: {string}, sendURL: {string} - should return error code {string} error message {string}', {
    timeout: parseInt(environment.MAX_TIMEOUT)
}, function(
    id,
    participant_bic,
    send_participant,
    send_participant_bic,
    federation_status,
    compliance_status_1,
    compliance_status_2,
    asset_code,
            ...
            if (err) {
                ...
                parser.parseString(encoder.decode(body.message, 'base64'), function(error, result) {
                    if (error === null) {
                        ...
                        // let errorMsg = result.Document.FIToFIPmtStsRpt[0].TxInfAndSts[0].StsRsnInf[0].Rsn[0].Prtry[0]
