...
# world-wire-service/auth-service/.credentials
...
--env (alias: -e) <local|dev|qa|st|prod> OPTIONAL
  ...
  ie: world-wire-dev, world-wire-qa, world-wire-st, world-wire-prod
...
deployPortal() {
    ...
    # ../world-wire-web/firebase-template.json is used to generate 
    # ../world-wire-web/firebase.json which is used to apply 
    # CSP headers in firebase, see docs:
    #  https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP
    #  https://firebase.google.com/docs/hosting/full-config#headers
    template_file="../world-wire-web/firebase-template.json" replace_file="../world-wire-web/firebase.json" template_txt="{{{PORTAL_DOMAIN}}}" replace_txt="$domain_short"  node ./project-provisioning/src/replace_from_template.js
...
deployMicroservices(){
    ...
    if [ $TRAVIS_BRANCH = 'dev-world-wire' ] && [ $TRAVIS_PULL_REQUEST = 'false' ]
    then
    ...
    elif [ $TRAVIS_BRANCH = 'qa-world-wire' ] && [ $TRAVIS_PULL_REQUEST = 'false' ]
    then
    ...
    elif [ $TRAVIS_BRANCH = 'staging-world-wire' ] && [ $TRAVIS_PULL_REQUEST = 'false' ]
    then
    ...
    elif [ $TRAVIS_BRANCH = 'test-world-wire' ] && [ $TRAVIS_PULL_REQUEST = 'false' ]
    then
        env="tn" # name of the folder in ./.credentials/{env} to use
        export REGION='us-east-2' && export CLUSTER_NAME='terraform-eks-ww-st' && aws eks update-kubeconfig --name $CLUSTER_NAME --region $REGION && kubectl -n default delete --all pods
    elif [ $TRAVIS_BRANCH = 'live-world-wire' ] && [ $TRAVIS_PULL_REQUEST = 'false' ]
    then
        env="prod"
...
setTravisDeployEnvVars()
{
    ...
    elif [ $TRAVIS_BRANCH = 'pen1-world-wire' ] && [ $TRAVIS_PULL_REQUEST = 'false' ]
    then
