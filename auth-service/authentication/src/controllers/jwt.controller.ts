...
// are secured is detailed here: https://github.ibm.com/gftn/world-wire-services/issues/77#issuecomment-10976927.
                        ...
                        if (!isUndefined(req.email) && req.email !== token_info.createdBy) {
                            ...
                            if (token_info.jti === body.jti) {
                                ...
                                jwt_infoRef.update(token_info).then(() => {
                                    ...
                                    this.setStatus(500);
                                    reject('unable to approve token');
                                });

                            } else {
                                self.setStatus(403);
                                reject('unknown token id');
                            }

                        } else {
                            self.setStatus(403);
                            reject('same user who created the token request cannot also approve');
                        }

                    } else {
                        self.setStatus(403);
                        reject('token is not currently under review');
                    }

                } else {
                    self.setStatus(404);
                    reject('token id info not found');
                }
