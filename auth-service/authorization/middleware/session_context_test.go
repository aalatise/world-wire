package middlewares

import (
	"net/http"
	"testing"

	. "github.com/smartystreets/goconvey/convey"
)

func TestGetIdentity1(t *testing.T) {

	jwtStr := "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6Ii1MZDMwM0V4a29vTkRRTVBoWWxLLjEtMTgifQ..123467890oiuytre"
	request, _ := http.NewRequest("GET", "", nil)
	request.Header.Set("Authorization", jwtStr)
	Convey("Successful get caller identity", t, func() {
		participantDomain, _ := GetIdentity(request)
		So(participantDomain, ShouldEqual, "participant2")
	})
}

func TestGetTimeTillExpired(t *testing.T) {

	jwtStr := "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6Ii1MZDMwM0V4a29vTkRRTVBoWWxLLjEtMTgifQ..123467890oiuytre"
	request, _ := http.NewRequest("GET", "", nil)
	request.Header.Set("Authorization", jwtStr)
	Convey("Successful parsed time validity", t, func() {
		timeTill, _ := GetTimeTill(request)
		So(timeTill, ShouldBeLessThan, 0)
	})
}

func TestHasAccountValidJWT(t *testing.T) {
	jwtStr := "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6Ii1MZDMwM0V4a29vTkRRTVBoWWxLLjEtMTgifQ..123467890oiuytre"
	request, _ := http.NewRequest("GET", "", nil)
	request.Header.Set("Authorization", jwtStr)
	Convey("Successful validated account name", t, func() {
		hasAccount := HasAccount("issuing", request)
		So(hasAccount, ShouldEqual, true)
		hasAccount = HasAccount("default", request)
		So(hasAccount, ShouldEqual, true)
		//invalidaccount
		hasAccount = HasAccount("defaultxyz", request)
		So(hasAccount, ShouldEqual, false)
	})
}
func TestHasAccountByPid(t *testing.T) {

	request, _ := http.NewRequest("GET", "", nil)
	request.Header.Set("x-fid", "test-fid")
	request.Header.Set("x-pid", "test-pid")
	request.Header.Set("x-iid", "test-iid")
	Convey("Successful skipped account name check for fid", t, func() {
		hasAccount := HasAccount("issuing", request)
		So(hasAccount, ShouldEqual, true)
		hasAccount = HasAccount("default", request)
		So(hasAccount, ShouldEqual, true)
		hasAccount = HasAccount("defaultxyz", request)
		So(hasAccount, ShouldEqual, true)
	})
}
func TestGetIdentity2(t *testing.T) {

	jwtStr := "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6Ii1MZDMwQVp2TjU3RDZ1YXpQcElxLjEtMCJ9..123467890oiuytre"
	request, _ := http.NewRequest("GET", "", nil)
	request.Header.Set("Authorization", jwtStr)
	Convey("Successful get caller identity", t, func() {
		participantDomain, _ := GetIdentity(request)
		So(participantDomain, ShouldEqual, "participant1")
	})
}

func TestGetIdentityByPid(t *testing.T) {

	request, _ := http.NewRequest("GET", "", nil)
	request.Header.Set("x-fid", "test-fid")
	request.Header.Set("x-pid", "test-pid")
	request.Header.Set("x-iid", "test-iid")
	Convey("Successful get caller identity", t, func() {
		participantDomain, _ := GetIdentity(request)
		So(participantDomain, ShouldEqual, "test-pid")
	})
}

func TestGetIdentityWithNoFID(t *testing.T) {
	jwtStr := "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6Ii1MZDMwQVp2TjU3RDZ1YXpQcElxLjEtMCJ9..123467890oiuytre"
	request, _ := http.NewRequest("GET", "", nil)
	request.Header.Set("Authorization", jwtStr)
	request.Header.Set("x-pid", "test-pid")
	request.Header.Set("x-iid", "test-iid")
	Convey("Successful get caller identity", t, func() {
		participantDomain, _ := GetIdentity(request)
		So(participantDomain, ShouldEqual, "participant1")
	})
}
