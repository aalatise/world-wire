...
class main {
    ...
    async init() {
        ...
        try {
            ...
            // add new firebase project name to world-wire-web/.firebaserc
    ...
    async createSecretsEnvs() {
        ...
        await new Promise(resolve => rl2.question(this.fmtN('FgBlue') + `\n\nRun the following in another terminal window \n\n$ cp -rf .credentials-v19/raw/dev/. ${this.credDir} ; rm ${this.credDir}/portal.txt ; cd ../world-wire-web/ ; firebase use ${this.envShortName} ; firebase apps:create WEB portal ;  firebase apps:sdkconfig -o ../auth-service/${this.credDir}/portal.txt \n\nPress enter to continue.`, ans => {
            rl2.close();
            resolve(ans);
        }));
    ...
    async deployAuthService() {
        ...
        //     // cmd
        //     let script = `export TRAVIS_BRANCH=${this.envShortName}-world-wire ; export TRAVIS_PULL_REQUEST=false ; bash -x ./deployment/deploy.sh --env ${this.envShortName}`
    ...
    async createBuildConfig() {
        ...
        const environmentJsonPath = `../world-wire-web/src/environments/environment.${this.envShortName}.ts`;
    ...
    inactivityTimeout: 15
        ...
        const angularJsonPath = '../world-wire-web/angular.json';
        ...
        const newAngularObj = _.set(angularJsonObj, 'projects.world-wire-web.architect.build.configurations.' + this.envShortName, config);
    ...
    async buildAndDeployPortal() {
        ...
        return await new Promise((resolve, reject) => {
            ...
            // cmd
            let script = `cd ../world-wire-web; ng build -c=${this.envShortName} ; firebase deploy --only hosting --project=${this.gcloudProjectName}`
    ...
    async addFirebaseUseConfig() {
        ...
        const firebaseRcPath = '../world-wire-web/.firebaserc';
    ...
    async applyFirebaseBoltRules() {
        ...
        return await new Promise((resolve, reject) => {
            ...
            // cmd
            let script = `cd ../world-wire-web; firebase-bolt database.rules.bolt ; firebase deploy --only database --project=${this.gcloudProjectName}`
    ...
    async deployFirebaseFunctions() {
        ...
        return await new Promise((resolve, reject) => {
            ...
            // cmd
            let script = `cd ../world-wire-web ; npm i ; cd functions ; npm i ; firebase use ${this.envShortName} ; firebase deploy --only functions`;
