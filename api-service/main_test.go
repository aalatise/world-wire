...
// 	os.Setenv(global_environment.ENV_KEY_FIREBASE_CREDENTIALS, common.Abs("./firebase/_next-world-wire-firebase-adminsdk-wvjz8-67ea263932.json"))
...
// 	})
// 	Convey("Error code & HTTP status code are returned as expected", t, func() {
// 		plBytes, _ := json.Marshal(model.Compliance{})
// 		req, _ := http.NewRequest("POST", "/"+serviceVersion+"/internal/compliance", bytes.NewBuffer(plBytes))
// 		response := executeRequest(req)
// 		msg := model.WorldWireError{}
// 		json.Unmarshal(response.Body.Bytes(), &msg)
// 		So(response.Code, ShouldEqual, http.StatusBadRequest)
// 		So(msg.Code, ShouldEqual, "API-1072")
// 	})
