...
func CreateAssetOperations() (AssetOperations, error) {
	...
	if strings.ToUpper(os.Getenv(global_environment.ENV_KEY_SECRET_STORAGE_LOCATION)) == common.HASHICORP_VAULT_SECRET {
		//Vault location
		ap.secret, err = vault.InitializeVault()
		if err != nil {
			LOGGER.Errorf("Error reading account source environment settings")
			return ap, err
		}
	} else {
		panic("No valid secret storage location specified")
	}
	...
	LOGGER.Infof("\t* Connecting Mongo DB ")
	...
	ap.DbSession = client
	LOGGER.Infof("\t* DB connection is ready")
...
func (ap AssetOperations) IssueAsset(w http.ResponseWriter, req *http.Request) {
	...
	issuingAccount, err := ap.secret.GetAccount(domainId, comn.ISSUING)
	if err != nil {
...
func (ap AssetOperations) AssetBalance(w http.ResponseWriter, req *http.Request) {
	...
	domainId := os.Getenv(global_environment.ENV_KEY_HOME_DOMAIN_NAME)
	account, err := ap.secret.GetAccount(domainId, accountName)
	if err == nil {
...
func (ap AssetOperations) AssetBalances(w http.ResponseWriter, req *http.Request) {
	...
	domainId := os.Getenv(global_environment.ENV_KEY_HOME_DOMAIN_NAME)
	account, err := ap.secret.GetAccount(domainId, comn.ISSUING)
	if err != nil {
		LOGGER.Errorf("Error while checking if issuing account exists: %v", err)
...
func (ap AssetOperations) TrustedAssetsForAccount(w http.ResponseWriter, req *http.Request) {
	...
	domainId := os.Getenv(global_environment.ENV_KEY_HOME_DOMAIN_NAME)
	account, err := ap.secret.GetAccount(domainId, distAccountName)
	if err != nil {
		LOGGER.Errorf("Error while checking if operating account exists: %v", err)
...
func (ap AssetOperations) allowTrust(trustRequest model.Trust, assetCode string, authorize bool, httpMethod string) error {
	...
	if isLocal {
		domainId := os.Getenv(global_environment.ENV_KEY_HOME_DOMAIN_NAME)
		trustorAccount, _ := ap.secret.GetAccount(domainId, accountName)
...
func (ap AssetOperations) changeTrust(trustRequest model.Trust, assetCode string, httpMethod string) error {
	...
	*/
	...
	if sourceAccountName != comn.ISSUING {
		...
		domainId := os.Getenv(global_environment.ENV_KEY_HOME_DOMAIN_NAME)
		acc, err := ap.secret.GetAccount(domainId, common.ISSUING)
		if err != nil {
	...
	domainId := os.Getenv(global_environment.ENV_KEY_HOME_DOMAIN_NAME)
	sourceAccount, err := ap.secret.GetAccount(domainId, sourceAccountName)
	if err != nil {
...
func (ap AssetOperations) CreateOrAllowTrust(w http.ResponseWriter, req *http.Request) {
	...
	if permission == "request" {
		...
		err = ap.storeTrustline(trustRequest)
		if err != nil {
			LOGGER.Debug("Update to db failed: ", err.Error())
		}
		return
...
func (ap *AssetOperations) storeTrustline(trustRequest model.Trust) error {
	LOGGER.Debugf("Writing trustline record to DB")
	homeDomain := os.Getenv(global_environment.ENV_KEY_HOME_DOMAIN_NAME)
	...
	collection, ctx := ap.DbSession.GetCollection()

	if permission == "request" {
		status := "requested"
		// Post Log to DB for trust request
		trustSuccess := Trustline{
			ID:          &endtoendId,
		...
		_, err := collection.InsertOne(ctx, trustSuccess)
		if err != nil {
			LOGGER.Debug("error updating trust line in mongo db: %s", err.Error())
			return err
		}
		...
		_, err := collection.UpdateOne(ctx, bson.M{"id": endtoendId}, bson.M{"$set": bson.M{
			"time_updated":    timeNow,
			"status":          status,
			"reason_rejected": *trustRequest.ParticipantID,
		}})
