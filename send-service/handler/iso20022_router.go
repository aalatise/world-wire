...
func iso20022Router(data []byte, messageType string, op message_handler.PaymentOperations, source string) ([]byte, error) {
	...
	if source == constant.REQUEST {
		...
		case constant.CAMT056:
			...
			if err != nil {
				LOGGER.Errorf("Constructing to go struct failed: %v", err.Error())
				report = op.ResponseHandler.CreateCamt030(BIC, "", "", constant.CAMT056, target, constant.STATUS_CODE_PARSE_FAIL)
				return report, err
			}
			...
			if err != nil {
				LOGGER.Errorf("Message payload validation failed: %v", err.Error())
				report = op.ResponseHandler.CreateCamt030(BIC, "", "", constant.CAMT056, target, constant.STATUS_CODE_REQ_VALIDATE_FAIL)
				return report, err
			}
		...
		case constant.CAMT026:
			...
			if err != nil {
				LOGGER.Errorf("Constructing to go struct failed: %v", err.Error())
				report = op.ResponseHandler.CreateCamt030(BIC, "", "", target, constant.CAMT026, constant.STATUS_CODE_PARSE_FAIL)
				return report, err
			}
			err = messageInstance.SanityCheck(BIC, target)
			if err != nil {
				LOGGER.Errorf("Message payload validation failed: %v", err.Error())
				report = op.ResponseHandler.CreateCamt030(BIC, "", "", target, constant.CAMT026, constant.STATUS_CODE_REQ_VALIDATE_FAIL)
				return report, err
			}
		...
		case constant.CAMT087:
			...
			if err != nil {
				LOGGER.Errorf("Constructing to go struct failed: %v", err.Error())
				report = op.ResponseHandler.CreateCamt030(BIC, "", "", target, constant.CAMT087, constant.STATUS_CODE_PARSE_FAIL)
				return report, err
			}
			err = messageInstance.SanityCheck(BIC, target)
			if err != nil {
				LOGGER.Errorf("Message payload validation failed: %v", err.Error())
				report = op.ResponseHandler.CreateCamt030(BIC, "", "", target, constant.CAMT087, constant.STATUS_CODE_REQ_VALIDATE_FAIL)
				return report, err
			}
		...
		case constant.CAMT029:
			...
			if err != nil {
				LOGGER.Errorf("Constructing to go struct failed: %v", err.Error())
				report = op.ResponseHandler.CreateCamt030(BIC, "", "", target, constant.CAMT029, constant.STATUS_CODE_PARSE_FAIL)
				return report, err
			}
			err = messageInstance.SanityCheck(BIC, target)
			if err != nil {
				LOGGER.Errorf("Message payload validation failed: %v", err.Error())
				report = op.ResponseHandler.CreateCamt030(BIC, "", "", target, constant.CAMT029, constant.STATUS_CODE_REQ_VALIDATE_FAIL)
				return report, err
			}
