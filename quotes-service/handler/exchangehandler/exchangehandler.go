...
import (
	...
	"github.com/IBM/world-wire/quotes-service/handler/helper"
	"github.com/IBM/world-wire/quotes-service/utility/cryptoservice"
	"github.com/IBM/world-wire/quotes-service/utility/gasservice"
	modeladaptor "github.com/IBM/world-wire/quotes-service/utility/modelAdaptor"
	...
	"github.com/IBM/world-wire/quotes-service/utility/whitelistservice"
...
func (eh *ExchangeHandler) CreateAtomicExchange(w http.ResponseWriter, request *http.Request) {
	...
	err = common.Retry(retryTimes, retryInterval*time.Second, func() error {
		...
		quoteResponse, _ := json.Marshal(*nqsExchangeRequest.ExchangeRequestDecode.Quote)
		...
		if err != nil {
			LOGGER.Warningf("Encounter error while updating record in DB %v, Re-establish connection now", err)
			eh.DBClient.CreateConnection()
		}
		return err
	})
	...
	collection, ctx := eh.LogDbClient.GetCollection()
	logBytes, err := json.Marshal(ExchangeLog)
	if err != nil {
		LOGGER.Error(err)
		return
	}
	var bdoc interface{}
	err = bson.UnmarshalJSON(logBytes, &bdoc)
	if err != nil {
		LOGGER.Error(err)
		return
	}
