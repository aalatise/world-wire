...
import (
	...
	pacs "github.ibm.com/gftn/iso20022/pacs00200109"

	pbstruct "github.ibm.com/gftn/iso20022/proto/github.ibm.com/gftn/iso20022/pacs00200109"
...
func (msg *Pacs002) SanityCheck(bic, participantId string) error {
	...
	if len(msg.Message.Body.TxInfAndSts) <= 0 || msg.Message.Body.TxInfAndSts[0].StsId == nil {
		return errors.New("Instruction ID is missing")
	}
	if err := parse.InstructionIdCheck(string(*msg.Message.Body.TxInfAndSts[0].StsId)); err != nil {
		return err
	}
...
func (msg *Pacs002) ProtobuftoStruct() (*sendmodel.XMLData, error) {
	...
	instructionId := string(*msg.Message.Body.TxInfAndSts[0].StsId)
